...
from swift.trainers import RLHFTrainerMixin
from swift.trainers.rlhf_trainer import patch_lora_merge, patch_lora_unmerge, round_robin
...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs: InputsType) -> InputsType:
        ...
        # print(total_rewards_per_func)
        # print(total_rewards_per_func.shape)
        # print('-' * 20)
        ...
        batch_encoded_inputs = self._prepare_batch_inputs(inputs, total_rewards,total_rewards_per_func)
        # Log metrics
    ...
    def _score_completions(self, inputs: InputsType) -> Tuple[torch.Tensor, torch.Tensor, List[str]]:
        ...
        # print(total_rewards_per_func, self.reward_weights)
    ...
    def _dynamic_sampling(self, inputs, rewards, rewards_per_func, completions):
        ...
        global_inputs = deepcopy(gather_object(inputs))
        for inp in global_inputs:
            inp['messages'].pop()
            del inp['is_truncated']

        all_sample_nums = len(rewards)//self.num_generations
        global_invalid_indexes = set([i for i in range(all_sample_nums)])
        ...
        while resample_count < self.args.max_resample_times:
            ...
            ## customized
            grouped_rewards = rewards_per_func[:,idx].view(-1, self.num_generations)         # 使用‘CLSAccuracyORM_choice’ reward
            # if grouped_rewards.get_device() == 0:
            #     print(f"grouped_rewards{grouped_rewards}")
            #     print('-'*10)

            # 检查是否存在大于0的值，如果不存在，则为无效组 ；True为不存在
            group_all_non_positive = (grouped_rewards <= 0).all(dim=1)

            # invalid_indices = torch.nonzero(group_all_non_positive, as_tuple=True)[0]  #第几个样本是有正确答案的
            valid_indices = torch.nonzero(~group_all_non_positive, as_tuple=True)[0]

            valid_mask = (~group_all_non_positive)

            # if valid_indices.get_device() ==0 :
            #     print(f"valid_indices{valid_indices}")
            #     print('-' * 10)
            #
            #     print(f"pre_validmask{valid_mask}")
            #     print('-' * 10)
            #
            #     print(f"local_sample_index{local_sample_index}")
            #     print('-' * 10)
            if local_sample_index is not None:
                this_valid_mask = torch.zeros_like(valid_mask, dtype=torch.bool)
                all_local_sample_index = torch.tensor(gather_object(local_sample_index), dtype=torch.int).to(valid_mask.device)   #样本id ， 没有self.num_generations个

                # if valid_indices.get_device() == 0:
                #     print(f"all_local_sample_index{all_local_sample_index}")
                #     print('-' * 10)
                global_valid_indices = all_local_sample_index[valid_indices]
                this_valid_indexes = set(global_valid_indices.tolist())


                # if valid_indices.get_device() == 0:
                #     print(f"this_valid_indexes{this_valid_indexes}")
                #     print('-' * 10)
                global_invalid_indexes = global_invalid_indexes - set(global_valid_indices.tolist())

                for this_valid_index in this_valid_indexes:             #为每一个idsample组选一个valid sample
                    # 条件计算布尔张量，结果中一定至少有一个True
                    result = (all_local_sample_index == this_valid_index) & valid_mask

                    # 找到第一个True的位置
                    true_indices = torch.nonzero(result, as_tuple=True)[0]
                    keep_index = true_indices[0]  # 获取第一个True的位置

                    # 将其他位置设置为False，保留第一个True
                    result[:] = False
                    result[keep_index] = True

                    this_valid_mask[result] = True

                valid_mask = this_valid_mask
            else:
                global_invalid_indexes = global_invalid_indexes - set(valid_indices.tolist())

            valid_mask = valid_mask.repeat_interleave(self.num_generations)
            ...
            if len(global_invalid_indexes) > 0:
                k = len(inputs)//self.num_generations
                invalid_indices_list = list(global_invalid_indexes)
                # 如果 true_indices 的元素数量大于等于 k
                if len(invalid_indices_list) >= k:
                    # 直接随机采样 k 个不重复的元素
                    local_sample_index = random.sample(invalid_indices_list, k)   #随机选择无效样本。
                else:  # 如果 true_indices 的元素数量不足 k
                    # 先取所有元素
        ...
        if len(valid_samples) >= self.effective_train_batch_size:
            ...
            #     print(f"rewards{torch.cat(valid_rewards)}")
            #     print(f"rewards_per_func{ torch.cat(valid_rewards_per_func)}")
            #     print(f"completions{valid_completions[:self.effective_train_batch_size]}")
        else:
            def resample_groups(data, group_size, k):
                """
                从原始 m 组数据扩展为 k 组数据，支持 list 和 torch.Tensor 数据类型。
            ...
            # if self.accelerator.process_index ==0:
            #     print(f"inputs{ resample_groups(valid_samples,self.num_generations,number_groups)}")
            #     print(f"inputs.shape{ len(resample_groups(valid_samples,self.num_generations,number_groups))}")
    ...
    def _prepare_batch_inputs(self, inputs: InputsType, rewards: torch.Tensor, rewards_per_func: List) -> List[InputsType]:
        """
        ...
        advantage_weight[advantage_weight==1]=0
        advantage_weight = advantage_weight + 1

        # print(grouped_answer_acc_rewards)
        # print(grouped_answer_acc_rewards.shape)
        # print(advantage_weight)
