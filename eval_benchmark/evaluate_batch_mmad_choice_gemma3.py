...
class MMADDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_one_shot:
            image_paths = [
                f"{os.path.join(self.root, text_gt['similar_templates'][0])}",
                f"{os.path.join(self.root, query_image_path)}",
            ]
            llm_input =[ 
            ...
            image_paths = [f"{os.path.join(self.root, query_image_path)}"]
            llm_input =[  
                {"role": "system", "content": [{"type": "text", "text": self.system_message}] },
             {
                "role": "user",
...
def run_inference(rank, world_size, args):
    ...
    # processor = AutoProcessor.from_pretrained(args.checkpoint, use_fast=True)
    ...
    dataloader = DataLoader(
        sampled_dataset, batch_size=1, collate_fn=custom_collate_fn, shuffle=False
    )
    ...
    with torch.no_grad():
        with tqdm(dataloader) as pbar:
            for llm_inputs, answer_entry in pbar:
                ...
                gpt_answers = [processor.decode(generated_ids[0][inputs["input_ids"].shape[1]:], skip_special_tokens=False)]
...
# python eval_benchmark/evaluate_batch_mmad_choice_gemma3.py --checkpoint /mnt/r-contentsecurity-p/common/checkpoints/yiyao/gemma-3-12b-it --data-root /mnt/vlr/laishi/IADdata/
...
if __name__ == "__main__":
    ...
    # run_inference(15,world_size,args)
