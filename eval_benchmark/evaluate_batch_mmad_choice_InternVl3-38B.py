...
def run(rank, world_size, args, ds_collection):
    ...
    model = CustomizedInternVLChatModel.from_pretrained(
        args.checkpoint,  low_cpu_mem_usage=True, torch_dtype=torch.bfloat16, device_map='auto').eval()
    ...
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False,
                            collate_fn=custom_collate_fn, sampler= sampler, num_workers=4)
        ...
        with tqdm(dataloader) as pbar:
            for llm_inputs, pixel_values, comp_pixel_values, use_comps, num_patches_list, answer_entry in pbar:
                ...
                if comp_pixel_values is not None:
                    comp_pixel_values = comp_pixel_values.to(torch.bfloat16).cuda()
...
if __name__ == '__main__':
    ...
    answer_entry_list = run(0,1,args, ds_collection)
