...
def run_inference(rank, world_size, args):
    ...
    if args.think:
        ...
        num_beams=1,  
        # num_beams=2,         #(4.51.1) Kimi-VL-A3B-Instruct is not support beam_search :NotImplementedError: Make sure that a `_reorder_cache` function is correctly implemented in transformers_modules.Kimi-VL-A3B-Instruct.modeling_kimi_vl to enable beam search for <class 'transformers_modules.Kimi-VL-A3B-Instruct.modeling_kimi_vl.KimiVLForConditionalGeneration'>
        max_new_tokens=1024,
        min_new_tokens=1,
        do_sample=True,
        temperature=0.5,
