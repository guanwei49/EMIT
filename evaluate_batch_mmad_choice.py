...
class MMADDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_one_shot and ('Defect' in question_type or question_type=='Anomaly Detection'):
        ...
        use_comp = len(image_path_list) == 2 and if_use_comp(llm_input, dataset_r)  ## Set if the soft prompt is adopted.
...
def run(rank, world_size, args, ds_collection):
    ...
    if args.peft is not None:
        ...
        model = PeftModel.from_pretrained(model, args.peft, is_trainable=False, torch_dtype=torch.bfloat16)
...
if __name__ == '__main__':
    ...
    ckpt_name = args.checkpoint.replace('/', '_')
    peft_name = str(args.peft).replace('/','_')

    if args.think:
        answers_json_path = os.path.join(args.out_dir, f'Answer_think_choice_{ckpt_name}_{peft_name}.json')
    else:
        answers_json_path = os.path.join(args.out_dir, f'Answer_choice_{ckpt_name}_{peft_name}.json')
