...
from model.internvl_chat.conversation import get_conv_template
...
class WeightedConcatDataset(ConcatDataset):   #weight的是不同训练集之间的weight
...
def build_transform(is_train, input_size, pad2square=False, normalize_type='imagenet'):
    ...
    if is_train:  # use data augumentation
        ...
        transform = T.Compose([
            T.Lambda(lambda img: img.convert('RGB') if img.mode != 'RGB' else img),
            T.RandomChoice([T.Lambda(jpeg_degrade_functions[quality]) for quality in qualities]),  #改质量  ，随机裁剪
            T.Resize((input_size, input_size), interpolation=InterpolationMode.BICUBIC),
            T.ToTensor(),
            T.Normalize(mean=MEAN, std=STD)
        ])
